          ...
          {siteConfig.stack.map((group, gi) => (
            <Reveal delay={(gi % 2) * 0.06} key={group.title}>
              <div className="relative border border-border bg-paper-2/70 p-6">
                <span
                  className="absolute -top-4 left-6 bg-accent-surface px-3.5 py-1 font-mono text-sm font-semibold uppercase tracking-[0.18em] text-on-accent"
                ...
                <div className="mt-3 flex flex-wrap gap-2.5">
                  {group.items.map((item, i) => (
                    <span
                      className="hand-chip inline-block rounded-sm border border-border bg-paper-2 px-3.5 py-2 font-serif text-lg text-foreground shadow-paper"
                      key={item}
